...
# def tokenize_and_align_labels(token, label):
...
# encode = tokenizer(token, is_split_into_words=True)

# data = tokenize_and_align_labels(token, label)
# label = torch.tensor(data['labels'])
# input_id = torch.tensor(data['input_ids'])
